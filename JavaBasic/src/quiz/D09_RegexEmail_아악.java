...
public class D09_RegexEmail_아악 {
	/*
	 1. emails 문자열에 있는 모든 이메일들을 ArrayList에 저장해보세요
	 2. 어떤 도메인(chosun.com, munhwa.com ..등) 이 몇 개씩 있는지 세서 출력해보세요
	...
	Pattern emailPattern = Pattern.compile("(\\w+)@(\\w+[.]\\w+([.]\\w+)*)"); //이렇게 해도 되지 않나? (내 생각)
	//Pattern emailPattern = Pattern.compile("(\\w+)@(\\w.\\w+(\\.\\w+)*)"); //엥 골뱅이 뒤에 이상한데. 결과는 맞긴 함 
	...
	//맵에는 정렬이 없음. 해시라 또 정렬이 없음 
	Map<String, Domain> domainCountMap = new HashMap<>();
	
	while(emailMatcher.find()) { //맵타입으로 개수 세주는 것도 나쁘지 않다 ㅏ...  머지 이거 
		...
		if (domain != null ) {
			//domainCountMap.get(domainName); get할 필요 없대
			domain.plus();
...
class Domain implements Comparable<Domain>{
	...
	public int compareTo(Domain d) {
		return d.count.compareTo(count); //Integer타입의 count라 compareTo쓸 수 있대 
		//이렇게 하면 내림차순 된대
		// return count.compareTo(d.count); 오름차순은 이거
	}
...
/*
내풀이.
		//아오 문제 똑바로 좀 읽자 이메일만 어레리에 넣으랬잖아!!△△△
		//내림차순 하는거 빼먹으니까 코드 방향이 완전히 달라짐..
		
	public static void main(String[] args) {
		//이메일 스트링을 List<String>으로변환 (변환은 했는데.. 바보여)
		System.out.println(emailToList(emails));
		
		//중복 있는 도메인 리스트
		List<String> domainList = domainList(emails);
		System.out.println(domainList);
		//중복제거한 도메인셋
		System.out.println(listToSet(domainList));
		Set<String> domainSet = listToSet(domainList(emails));
		
		System.out.println(cntDomain(domainList, domainSet));
	}
	//이메일을 ArrayList에 담기
	public static List<String> emailToList(String emails) {
		String[] strArr = emails.split("\n");
		List<String> list = Arrays.asList(strArr);
		return list;
	}
	//도메인 목록(중복 ㅇ) ArrayList에 담기
	public static List<String> domainList(String str){
		Pattern pattern = Pattern.compile("\\w+[.]com");
		Matcher matchResult = pattern.matcher(str);

		ArrayList<String> arrList = new ArrayList<>();
		while(matchResult.find()) {
			arrList.add(matchResult.group());
		}
		return arrList; 
	}
	//중복 있는 도메인 리스트를 Set에 담아 중복 없애기
	public static Set<String> listToSet(List<String> list) {
		HashSet<String> set = new HashSet<>(list);
		return set;
	}
	//도메인 개수 세기 
	public static HashMap<String, Integer> cntDomain(List<String> emailList, Set<String> domains) {
		HashMap<String, Integer> map = new HashMap<>();
		for(String domain : domains) {
			map.put(domain, Collections.frequency(emailList, domain));
		}
		return map;
	}
	
	//아 해시맵 값기준으로 정렬 어떻게 하지...?? △△△
 */
